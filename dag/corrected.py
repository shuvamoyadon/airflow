...
def generate_merge_sql(**kwargs):
    ...
    # ADDED — Load non_json_key block
    non_json_lookup = {m["source_field"]: m.get("target_field", "") 
                       for m in config_data.get("non_json_key", [])}
    print("Non-JSON mapping loaded:", non_json_lookup)
    ...
    for col in source_cols:
        ...
        # ADDED — Handle non-JSON fields
        if col in non_json_keys:
            expr = f"SAFE_CAST({col} AS {col_type}) AS {col}"
